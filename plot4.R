...
plot4 <- function() {
    ...
    # 4 plots will be placed on the device, row by row
    par(mfrow = c(2, 2))
    ...
    plot(dd$Time, dd$Voltage, type = "l",
         ylab = "Voltage", xlab = "datetime")
        
    # Third plot - lower left
    plot(dd$Time, dd$Sub_metering_1, type = "l",
         ylab = "Energy sub metering", xlab = "")
    points(dd$Time, dd$Sub_metering_2, type = "l", col = "red")
    points(dd$Time, dd$Sub_metering_3, type = "l", col = "blue")
    legend("topright", col = c("black", "red", "blue"), 
           legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"),
           lwd=1, lty=c(1, 1, 1), bty = "n")
    
    # Fourth plot - lower right
    plot(dd$Time, dd$Global_reactive_power, type = "l",
